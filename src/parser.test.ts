...
import { parseCanvas, CanvasParseError } from "../src/parser";
...
test("parses all four node types", () => {
  const json = JSON.stringify({
    nodes: [
      ...
      { id: "f1", type: "file", x: 0, y: 0, width: 100, height: 100, file: "Note.md", subpath: "#heading" },
      { id: "l1", type: "link", x: 0, y: 0, width: 100, height: 100, url: "https://example.com" },
      { id: "g1", type: "group", x: 0, y: 0, width: 100, height: 100, label: "Group", background: "bg.png", backgroundStyle: "cover" },
    ],
    edges: [],
  });
...
test("parses optional fields correctly", () => {
  const json = JSON.stringify({
    ...
    edges: [
      { id: "e1", fromNode: "n1", toNode: "n1", fromSide: "right", toSide: "left", fromEnd: "arrow", toEnd: "none", color: "#ff0000", label: "test" },
    ],
  });
  const data = parseCanvas(json);
...
test("edge defaults: missing fromSide/toSide/fromEnd/toEnd are undefined", () => {
  const json = JSON.stringify({
    nodes: [
      { id: "n1", type: "text", x: 0, y: 0, width: 100, height: 100, text: "hi" },
    ],
    edges: [
...
test("throws on invalid fromSide", () => {
  const json = JSON.stringify({
    ...
    edges: [
      { id: "e1", fromNode: "n1", toNode: "n1", fromSide: "center" },
    ],
  });
...
test("throws on invalid toSide", () => {
  const json = JSON.stringify({
    ...
    edges: [
      { id: "e1", fromNode: "n1", toNode: "n1", toSide: "middle" },
    ],
  });
...
test("throws on invalid fromEnd", () => {
  const json = JSON.stringify({
    ...
    edges: [
      { id: "e1", fromNode: "n1", toNode: "n1", fromEnd: "circle" },
    ],
  });
...
test("throws on invalid toEnd", () => {
  const json = JSON.stringify({
    ...
    edges: [
      { id: "e1", fromNode: "n1", toNode: "n1", toEnd: "diamond" },
    ],
  });
...
test("throws when edge references nonexistent node", () => {
  const json = JSON.stringify({
    ...
    edges: [
      { id: "e1", fromNode: "n1", toNode: "ghost" },
    ],
  });
...
test("throws when edge references nonexistent fromNode", () => {
  const json = JSON.stringify({
    ...
    edges: [
      { id: "e1", fromNode: "ghost", toNode: "n1" },
    ],
  });
...
test("color can be hex, preset, or rgb", () => {
  const json = JSON.stringify({
    nodes: [
      ...
      { id: "n3", type: "text", x: 0, y: 0, width: 100, height: 100, text: "c", color: "rgb(255,0,0)" },
    ],
    edges: [],
  });
  const data = parseCanvas(json);
...
test("subpath is optional on file nodes", () => {
  const json = JSON.stringify({
    nodes: [
      { id: "n1", type: "file", x: 0, y: 0, width: 100, height: 100, file: "Note.md" },
    ],
    edges: [],
  });
  const data = parseCanvas(json);
...
test("parses real Demo.canvas file", async () => {
  ...
  expect(data.nodes.map(n => n.type)).toEqual(["group", "text", "text", "file", "link"]);
});
